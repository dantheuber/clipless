...
- **Optional usage counts** — Settings → General → Privacy → Share usage counts (off by default). Sends active days and a random installation ID to PostHog; no clipboard contents, configured tools, session recordings, or error reports. PostHog sees the connection's IP address. [What is collected and how to opt out](src/main/analytics/README.md).
